...
public class CSC extends JavaPlugin {
	...
	public void onEnable() {
		...
		if(data[3].equals("UNSET") || data[4].equals("UNSET")) {
			Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', loc.getString("UnsetValues")));
			return;
		}
		try {
	...
	private void workData() throws IOException {
        ...
        if (remove == true) {
        	if(data.delete()){
        		Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', loc.getString("DataRemoved")));
			} else Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', loc.getString("DataRemoveNotFound")));
        } else {
    		loadData();
        }
	}
	...
	public String[] loadConfig() {
        ...
        try {
    		...
    		loc.init();
    		Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', loc.getString("ConfigLoaded")));
        } catch(IOException e) {
            e.printStackTrace();
        }
        return data;
	...
	private void saveData() {
		try{
			...
			ps.close();
			Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', loc.getString("DataSaved")));
		}catch(IOException e){
			e.printStackTrace();
		}
	}
		...
		try{
			...
			if(file.exists()) {
				...
				try {
					...
					Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', loc.getString("DataLoaded")));
				} finally {
					br.close();
				}
			} else {
			    Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&', loc.getString("DataNotfound")));
			}
		} catch(IOException e) {
			e.printStackTrace();
		}
